...
import { useCandidate, useCandidateTimeline, useUpdateCandidate } from '../../hooks/useCandidates';
...
const Card = styled.div`
  ...
  box-shadow: 0 8px 28px rgba(59,130,246,0.12), 0 2px 8px rgba(139,92,246,0.08);
  transition: all 0.25s;
  &:hover {
    transform: translateY(-4px) scale(1.01);
    box-shadow: 0 12px 36px rgba(139,92,246,0.18), 0 4px 14px rgba(59,130,246,0.12);
  }
`;
...
export default function CandidateProfile() {
  ...
  if (!candidate) return <div>Candidate not found. <Link to="/candidates">Back to Candidates</Link></div>;
  ...
  async function addNote() {
    ...
    await update.mutateAsync({ id: candidate.id, updates: { notes: updatedNotes } });
    setNote('');
  }
  ...
  return (
    <Wrap>
      ...
      <div style={{ color: '#475569', fontSize:18, fontWeight:500, marginBottom:8 }}>
        {candidate.email}
      </div>
        ...
        <Card>
          <Row>
            <div style={{fontWeight:600, fontSize:17}}>
              <strong>Stage:</strong>{' '}
              <span style={{textTransform:'capitalize', color:'#3b82f6'}}>{candidate.stage}</span>
            </div>
            <div>
              <Select
          ...
          <div style={{marginTop:18}}>
            ...
            {/* Notes List */}
            <div style={{marginTop:18}}>
              {(candidate.notes || []).slice().reverse().map((n: Note) => (
